...
from ..helper.relative_kernel import *
from ..helper.sn_eig_solver import *
import collections
...
class sknet(algorithm):
	...
	def update_U(self, network_name='knet'):
		...
		DKxD = torch.from_numpy(DKxD)
		DKxD = Variable(DKxD.type(db['dataType']), requires_grad=False)


		#[db['U'], db['U_normalized']] = L_to_U(db, DKxD)
	...
	def initialize_U(self):
		...
		print(np.max(db['U_normalized'] - db['relative_K'].U_normalized))
